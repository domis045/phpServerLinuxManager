<?php
   ...
   switch(strtolower($_SESSION['kalba'])){
	case "samp":
		$EXE = "samp03svr";
		$GAME = strtolower($_SESSION['kalba']);
		$DIR = "/servers/".$GAME.$_SESSION['id'];
		//$DIR = "/servers/samp".+$_SESSION['id'];
		break;
}
   //-----------------------------------------//
   ?>
...
<html >
   ...
   <body>
	...
	<a href='index.php'><button type='submit' id='button' class='float' align='center'>Atsijungti</button></a><hr>
         <p>
	<?php serverControl(start, $DIR, $EXE, strtolower($_SESSION['kalba'].+$_SESSION['id'])); ?>
